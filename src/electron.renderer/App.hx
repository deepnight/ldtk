...
class App extends dn.Process {
	...
	static inline function get_APP_ASSETS_DIR()
		return APP_RESOURCE_DIR + "assets/";

	public var jDoc(get, never):J;

	inline function get_jDoc()
		return new J(js.Browser.document);

	public var jBody(get, never):J;

	inline function get_jBody()
		return new J("body");

	public var jPage(get, never):J;

	inline function get_jPage()
		return new J("#page");

	public var jCanvas(get, never):J;

	inline function get_jCanvas()
		return new J("#webgl");
	...
	public function new() {
		...
		// Keyboard events
		jBody.on("keydown", onJsKeyDown).on("keyup", onJsKeyUp);
	...
	public static inline function isLinux()
		return js.node.Os.platform() == "linux";

	public static inline function isWindows()
		return js.node.Os.platform() == "win32";

	public static inline function isMac()
		return js.node.Os.platform() == "darwin";

	public inline function isKeyDown(keyId:Int)
		return jsKeyDowns.get(keyId) == true || heapsKeyDowns.get(keyId) == true;

	public inline function isShiftDown()
		return isKeyDown(K.SHIFT);

	public inline function isCtrlDown() {
		return App.isMac() ? jsMetaKeyDown || isKeyDown(91) || isKeyDown(93) : isKeyDown(K.CTRL);
	}

	public inline function isAltDown()
		return isKeyDown(K.ALT);

	public inline function hasAnyToggleKeyDown()
		return isShiftDown() || isCtrlDown() || isAltDown();

	var _inputFocusCache:Null<Bool> = null;

	public inline function hasInputFocus() {
		if (_inputFocusCache == null)
			_inputFocusCache = jBody.find("input:focus, textarea:focus, .cm-editor").length > 0;
	...
	function clearMiniNotif() {
		jBody.find("#miniNotif").stop(false, true).fadeOut(1500);
	}
	...
	function onGlContextLoss() {
		...
		var m = Editor.exists() ? new ui.modal.dialog.Warning(L.t._("The WebGL context was lost!\nDon't worry, it's probably nothing, and no data was lost. You should just save your work and restart the application.")) : new ui.modal.dialog.Warning(L.t._("The WebGL context was lost!\nYou need to restart the application."));
	...
	function clearCurPage() {
		jPage.empty().off().removeClass("locked");
	...
	public function loadProject(filePath:String, ?levelIndex:Int, ?onComplete:(p:Null<data.Project>) -> Void):Void {
		new ui.ProjectLoader(filePath, (p) -> {
			if (onComplete != null)
				onComplete(p);
	...
	override function update() {
		...
		if (cd.has("debugTools")) {
			...
			debugPre("appButtons=" + (isMouseButtonDown(0) ? "[left] " : "") + (isMouseButtonDown(2) ? "[right] " : "")
				+ (isMouseButtonDown(1) ? "[middle] " : "") + " toggles=" + (isCtrlDown() ? "[ctrl] " : "") + (isShiftDown() ? "[shift] " : "")
				+ (isAltDown() ? "[alt] " : ""));
