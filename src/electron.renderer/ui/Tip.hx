...
class Tip extends dn.Process {
	...
	private function new(?jTarget:js.jquery.JQuery, str:String, ?keys:Array<Int>, ?className:String, forceBelowPos=false) {
		...
		if( jTarget!=null ) {
			...
			var y = tOff.top + jTarget.outerHeight() + 4;
			...
			if( jTarget.attr("tip")!=null ) {
				switch jTarget.attr("tip") {
					case "left":
						x = tOff.left - jTip.outerWidth();
						y = tOff.top;

					case "right":
						x = tOff.left + jTarget.outerWidth();
						y = tOff.top;

					case "top":
						x = tOff.left;
						y = tOff.top - jTip.outerHeight();

					case "bottom":
						x = tOff.left;
						y = tOff.top + jTarget.outerHeight();
				}
			}
